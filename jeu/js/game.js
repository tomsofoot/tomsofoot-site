...
(function (global) {
  ...
  async function pick(id) {
    ...
    try {
      ...
      var finish = function () {
        ...
        if (r.isWin) { showBravo(); cxNotifyFinished("completed"); }
      };
  ...
  if (revealBtn) revealBtn.addEventListener("click", function () {
    ...
    API.revealAnswer().then(function (r) {
      ...
      refreshPoints(); scrollToEnd(); cxMountReopen(); cxNotifyFinished("revealed");
    });
  });
  ...
  // officielle (jamais à la reprise). Ne touche ni au score, ni aux points, ni au serveur.
  function cxNotifyFinished(endState) {
    if (!global.TomsoFootContinue) return;
    try { global.TomsoFootContinue.setSelf("daily-player"); global.TomsoFootContinue.notifyFinished("daily-player", { state: endState || "completed" }); } catch (e) {}
  }
  function cxMountReopen() {
    if (!global.TomsoFootContinue || !el.end) return;
    var card = el.end.querySelector(".end-card");
    try { if (card) global.TomsoFootContinue.mountReopenButton(card, "daily-player"); } catch (e) {}
  }
  // Reprise : reflète l'état officiel (serveur) dans la balise du jour, SANS ouverture auto.
  function cxMarkResume(endState) {
    if (!global.TomsoFootContinue) return;
  ...
  async function rebuildFromResume(s) {
    ...
    if (state.status === "won") { showBravo(); cxMarkResume("completed"); }
      ...
      refreshPoints(); cxMountReopen(); cxMarkResume("revealed");
    }
  }
